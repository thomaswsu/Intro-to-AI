...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated so
             that the ghost appears in its prison cell, position self.getJailPosition() Done?
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                allPossible[p] = emissionModel[trueDistance] * self.beliefs[p] # update probaility 

        "*** END YOUR CODE HERE ***"

        allPossible.normalize()
        self.beliefs = allPossible # Note that its updated here so you don't have to do it yourself.
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        self.beliefs = allPossible # Whoops don't forget to update this or else you just throw away all your work.
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for particle in range(self.numParticles):
            spot = particle % len(self.legalPositions)
            self.particles.append(self.legalPositions[spot])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            particlesBeliefs = util.Counter()

            for particle in self.particles:
                particlesBeliefs[particle] += emissionModel[util.manhattanDistance(pacmanPosition, particle)]
            particlesBeliefs.normalize()
            if particlesBeliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                particles = []
                for _ in range(len(self.particles)):
                    particles.append(util.sample(particlesBeliefs))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            beliefDistribution[particle] += float(1)
